import { facebookRegisterLogin, githubRegisterLogin, googleRegisterLogin, kakaoRegisterLogin } from "@api/social"
import Kakao from "@pages/callback/Kakao"
import { Facebook, Github, Google, KaKao } from "@styles/login.style"
import { MypageCompTitle } from "@styles/mypage.style"
import styled from "styled-components"

const SNSRegisterWrapper = styled.div`
  max-width: 380px;
  margin: 0 auto;
  display: flex;
...
function SNSRegister () {
  ...
  const onKakaoRegister = () => {
    location.href = kakaoRegisterLogin
  }

  const onGoogleRegister = () => {
    location.href = googleRegisterLogin
  }

  const onGithubRegister = () => {
    location.href = githubRegisterLogin
  }

  const onFacebookRegister = () => {
    location.href = facebookRegisterLogin
  }

  return (
    <SNSRegisterWrapper>
      <MypageCompTitle>SNS 등록</MypageCompTitle>
      <SNSList>
        <KaKao onClick={onKakaoRegister}>
        ...
        <Google onClick={onGoogleRegister}>
        ...
        <Github onClick={onGithubRegister}>
        ...
        <Facebook onClick={onFacebookRegister}>
